...
|------|--------|---------|-------|-------|
| 📡 | **[Attach Elastic IP to EC2 Instance](aws-scripts/1-attach-elastic-ip-ec2.sh)** | Associates an Elastic IP (EIP) with an EC2 instance by retrieving IDs via tags and associating them. | Beginner | `./aws-scripts/1-attach-elastic-ip-ec2.sh <REGION> <INSTANCE_NAME> <EIP_NAME>` |
| 🔌 | **[Attach ENI to EC2 Instance](aws-scripts/2-attach-eni-to-ec2.sh)** | Attaches a Network Interface (ENI) to an EC2 instance, waits for initialization, and confirms attachment. | Beginner | `./aws-scripts/2-attach-eni-to-ec2.sh <REGION> <INSTANCE_NAME> <ENI_NAME>` |
| 💾 | **[Attach Volume to EC2 Instance](aws-scripts/3-attach-volume-to-ec2.sh)** | Attaches an EBS volume to an EC2 instance by retrieving IDs via tags and attaching it at a specified device. | Beginner | `./aws-scripts/3-attach-volume-to-ec2.sh <REGION> <INSTANCE_NAME> <VOLUME_NAME> <DEVICE_NAME>` |
| 🖼️ | **[Create AMI from EC2 Instance](aws-scripts/4-create-ami-from-ec2.sh)** | Creates an Amazon Machine Image (AMI) from a running or stopped EC2 instance and waits for it to be available. | Intermediate | `./aws-scripts/4-create-ami-from-ec2.sh <REGION> <INSTANCE_NAME> <AMI_NAME>` |
...
| 📸 | **[Create Volume Snapshot](aws-scripts/6-create-volume-snapshot.sh)** | Creates a snapshot of an EBS volume with a given description and waits for completion. | Beginner | `./aws-scripts/6-create-volume-snapshot.sh <REGION> <VOLUME_NAME> <SNAPSHOT_NAME> <DESCRIPTION>` |
| 🔄 | **[Create Volume from Snapshot](aws-scripts/7-create-volume-from-snapshot.sh)** | Creates a new EBS volume from an existing snapshot in a specified availability zone and waits for availability. | Beginner | `./aws-scripts/7-create-volume-from-snapshot.sh <REGION> <SNAPSHOT_NAME> <NEW_VOLUME_NAME> <AVAILABILITY_ZONE>` |
...
| 💰 | **[Create Budget via REST API](azure-scripts/4-create-budgets-restapi.sh)** | Creates an Azure budget with email notifications using the REST API and verifies the budget. | Intermediate | `./azure-scripts/4-create-budgets-restapi.sh <BUDGET_NAME> <AMOUNT> <EMAIL_ADDRESS>` |
...
| 🏗️ | **[Create Subnet in Virtual Network](azure-scripts/6-create-subnets-network.sh)** | Creates an Azure Virtual Network with a subnet and verifies the network setup. | Beginner | `./azure-scripts/6-create-subnets-network.sh <VNET_NAME> <SUBNET_NAME> <LOCATION> <VNET_PREFIX> <SUBNET_PREFIX>` |
